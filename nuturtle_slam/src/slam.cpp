/// \file
/// \brief Node for the EKF-SLAM algorithn
///
/// PUBLISHES:
...
///     landmarks (nuslam/TurtleMap): subscribe the landmarks with unknown data associations
...
public:
    SLAM(ros::NodeHandle &nh)
    {
        ...
        joint_state_sub_ = nh.subscribe("joint_state", 1000, &SLAM::joint_states_callback, this);
        landmarks_sub_ = nh.subscribe("fake_landmarks", 1000, &SLAM::landmarks_callback, this);
    ...
    void landmarks_callback(const nuturtle_slam::TurtleMap &msg)
    {
        if (if_measurements_ == false)
        {
            ...
            {
                int id_associated = data_association(msg, i);
                if (id_associated == -1)
                {
                    continue;
                }
                ...
                measurements_.emplace_back(id_associated, range, bearing);
                // std::cout << "Measured Range at " << id_associated << " is " << range << std::endl;
                // std::cout << "Measured Bearing at " << id_associated << " is " << bearing << std::endl;
            }
    ...
    /// \brief data association: return the id of this landmark
    ///
    ///     either associate with an existing landmarks or create a new landmark
    ///     ith measurement and jth landmark
    int data_association(const nuturtle_slam::TurtleMap &msg, int i)
    {
        int landmark_numbers = (mu_.size() - 3) / 2;
        bool matched = false;
        for (int j = 0; j < landmark_numbers; j++)
        {
            double landmark_x = mu_(3 + j * 2);
            double landmark_y = mu_(4 + j * 2);

            // transfer back to robot coordinate
            double inv_x = -mu_(0) * std::cos(mu_(2)) - mu_(1) * std::sin(mu_(2));
            double inv_y = mu_(0) * std::sin(mu_(2)) - mu_(1) * std::cos(mu_(2));
            double inv_theta = -mu_(2);
            ...
            double distance = std::sqrt(std::pow((result_x - msg.x.at(i)), 2) +
                                        std::pow((result_y - msg.y.at(i)), 2));
            ...
            if (distance < 0.3)
            {
                for (auto const &map : landmark_id_to_index_)
                {
                    if (map.second == (3 + j * 2))
                    {
                        return map.first;
